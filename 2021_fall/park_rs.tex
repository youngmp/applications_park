...
\begin{document}
    ...
    %An important problem I have found is that the stochastic and deterministic models may not agree in two ways. First, stochastic and deterministic models may have similar steady-state distributions, but do not necessarily have the same dynamics. For example, stochastic models of population growth may result in extinction even though the steady-state distribution exhibits a non-zero mean (Keizer's paradox). Second, adding noise to a deterministic model does not necessarily reproduce the dynamics of the stochastic model. In fact, Langevin equations are known to fail to reproduce dynamics of the corresponding stochastic simulation if birth and death rates are not similar.
